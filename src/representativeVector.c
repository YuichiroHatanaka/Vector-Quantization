...
int representativeVector(FILE *block_fp, FILE *codebook_fp, FILE *vq_fp, unsigned char raw_vector_data[], int vector_element, int *representative_vector_number){
	unsigned char representative_vector_data[vector_element];
	//マップが大きいためint
	int *temporary_data = representative_vector_number;

	fseek(block_fp, -vector_element, SEEK_CUR);
	fread(raw_vector_data, sizeof(char), vector_element, block_fp);

	//iもintにする
	for(int i = 0; i < *temporary_data ; i++){
...
//			printf("data is %d new!", *temporary_data);
			...
			fwrite(&*temporary_data, sizeof(int), 1, vq_fp);
